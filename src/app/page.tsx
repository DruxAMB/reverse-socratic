...
export default function Home() {
  ...
  const handleTryDemo = () => {
    ...
    // Scroll to the picker section after it renders
    requestAnimationFrame(() => {
      const prefersReducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
      if (prefersReducedMotion) {
        pickerRef.current?.scrollIntoView({ behavior: "auto" });
        return;
      }

      // Animate the picker section in
      const items = gsap.utils.toArray<HTMLElement>("[data-picker-item]", pickerRef.current);
  ...
  return (
    <div className="flex flex-col flex-1 bg-background">
      {/* Hero — fills the viewport, no scroll */}
      <section
        ref={heroRef}
